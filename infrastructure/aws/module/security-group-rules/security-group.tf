...
resource "aws_security_group_rule" "ingress-rules" {
  ...
  count = var.create ? length(var.ingress-rules) : 0}

  security_group_id = var.sg-id
  type              = "ingress"

  cidr_blocks     = [ split(",", lookup(var.ingress-rules[count.index], "cidr_blocks", join(",", var.ingress-cidr))) ]
  description     = lookup(var.ingress-rules[count.index], "description", var.ingress-description)

  from_port = lookup(var.ingress-rules[count.index], "from-port" )
  to_port   = lookup(var.ingress-rules[count.index], "to-port" )
  protocol  = lookup(var.ingress-rules[count.index], "protocol" )
