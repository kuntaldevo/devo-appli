...
variable "ingress-cidr" {
  description = "List of IPv4 CIDR ranges to use on all ingress rules, where each element in the list is a map defining a CIDR and description"
  type = list(map(string))
  default     = []
}

variable "ingress-rules" {
  description = "List of ingress rules to create where 'cidr_blocks' is used. Each element in the list is a map defining the Protocol, Port Range, Source CIDR and the optional Description"
  type = list(map(string))
  default     = []
}
