...
data "null_data_source" "ingress-join" {
  ...
  inputs = {
    rule-number = count.index
    cidr-block = lookup ( var.ingress-cidr[count.index / local.ingress-rules-size], "cidr" )
    description = lookup ( var.ingress-cidr[count.index / local.ingress-rules-size], "description" )
