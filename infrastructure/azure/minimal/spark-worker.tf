
variable "spark-worker-name" {
  type = "string"
  default = "spark-worker"
}


resource "azurerm_virtual_machine" "spark-worker" {

  name                = "${var.env-key}.${var.spark-worker-name}"
  location            = "${var.region-id}"
  resource_group_name = "${azurerm_resource_group.default.name}"
  depends_on = ["azurerm_virtual_machine.spark-master"]

  network_interface_ids = ["${azurerm_network_interface.spark-worker.id}"]
  vm_size               = "Standard_DS1_v2"

  storage_image_reference {
    id = "/subscriptions/${var.subscription-id}/resourceGroups/${var.vmi-resource-group}/providers/Microsoft.Compute/images/${var.spark-worker-name}"
  }

  storage_os_disk {
    name              = "${var.spark-worker-name}.os-disk"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Premium_LRS"
  }

# OS_PROFILE's computer_name will set the host name of the VM
# The rest of the OS_PROFILE appears to be required, so I've set it to something, but it's not used for anything
  os_profile {
    computer_name  = "${var.spark-worker-name}"
    admin_username = "azureuser"
  }

  os_profile_linux_config {
    disable_password_authentication = true
        ssh_keys {
            path     = "/home/azureuser/.ssh/authorized_keys"
            key_data = "ssh-rsa  fake-key"
        }
  }

  boot_diagnostics {
    enabled     = "true"
    storage_uri = "${azurerm_storage_account.logs.primary_blob_endpoint}"
  }

  tags {
      cluster-id   = "${var.cluster-id}"
      tf-config = "${var.tf-config}"
      tf-resource = "azurerm_virtual_machine.spark-worker"
   }
}
